...
- API-client response shape MUST match how you read it (the #1 silent-login bug). A \`fetch\` wrapper that does \`return res.json()\` returns the BODY directly — so read fields directly: \`const { token, user } = await api.post('/auth/login', …)\`. Do NOT read \`response.data.token\`/\`response.data.user\` from a fetch client (that's the axios shape — \`.data\` is undefined, so the token is never stored and login silently fails with no redirect). Either keep the fetch client returning the body and read fields directly, OR make the client return \`{ data: await res.json() }\` and read \`.data\` everywhere — but be consistent across the WHOLE app.

Server init & seeding (CRITICAL — production runs under a WSGI/process manager, not \`python app.py\`):
- .NET / ASP.NET: read the DB connection from \`ConnectionStrings:DefaultConnection\` (the platform injects \`ConnectionStrings__DefaultConnection\` as an Npgsql key-value string for managed Postgres — Npgsql can't parse a \`postgresql://\` URL), and bind Kestrel to the injected port via \`ASPNETCORE_URLS=http://+:$PORT\`. Spring: use \`SPRING_DATASOURCE_URL\` (\`jdbc:postgresql://…\`) + username/password, and \`server.port=\${PORT}\`.
