...
- Databases: default to SQLite so it runs in the preview (Prisma \`provider = "sqlite"\`, \`url = env("DATABASE_URL")\`, plus a \`.env\` with \`DATABASE_URL="file:./dev.db"\`). Read the URL from \`process.env.DATABASE_URL\` — never hardcode credentials — so a managed Postgres can be swapped in at deploy time.
